...
def hefty_hogs(player_score, opponent_score):
    ...
    while opponent_score>0:
        function = digit_fn(opponent_score%10)
        player_score = function(player_score)
        opponent_score//=10
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        if last_leader == 0:
            ...
            return 0, "Player 0 takes the lead by " + str(lead)
    elif score1> score0:
        lead = score1 - score0
        if last_leader == 1:
            return 1, None
        else:
            return 1, "Player 1 takes the lead by " + str(lead)
    else:
        return None, None
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    min = 1
    while i <= 10:
        current_average = averaged_rolls(i, dice)
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if (hefty_hogs_score + score)%10 == opponent_score%10:
        if hog_pile(hefty_hogs_score+score, opponent_score) > 0:
            return 0
        else:
            return num_rolls
